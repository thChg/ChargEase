...
export const fetchChargingStations = createAsyncThunk( "/charger/findCharging",
  async (accessToken : string, {rejectWithValue}) => {
    try {
      ...
      return rejectWithValue(error.response?.data?.message || "Failed to fetch charging stations.");
    }
  }
)
...
const ChargingStationSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchChargingStations.fulfilled, (state, action: PayloadAction<ChargingStation>) => {
        state.loading = false;
        state.chargingStations = Array.isArray(action.payload) ? action.payload : [action.payload];
      })
